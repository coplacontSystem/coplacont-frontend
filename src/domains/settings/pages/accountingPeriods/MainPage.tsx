...
import { ESTADO_PERIODO_OPTIONS, formatPeriodoLabel } from "../../types";
...
export const MainPage: React.FC = () => {
 ...
 const [selectedPeriodo, setSelectedPeriodo] =
  useState<ConfiguracionPeriodo | null>(null);
 ...
 const fetchPeriodos = async () => {
  try {
   ...
   setError(
    "Error al obtener periodos. Por favor, intente nuevamente, " + error,
   );
   setIsError(true);
   setPeriodos([]);
  } finally {
 ...
 const handleCreate = async () => {
  ...
  try {
   ...
   setError(
    "Error al crear el periodo. Verifique que no exista un periodo duplicado.",
   );
  } finally {
   setIsLoading(false);
  }
