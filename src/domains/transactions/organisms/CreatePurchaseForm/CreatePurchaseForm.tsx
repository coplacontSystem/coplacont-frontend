...
import { useGetInventoryByWarehouseQuery } from "@/domains/inventory/api/inventoryApi";
import type {
 Entidad,
 EntidadParcial,
...
import type { RegisterPurchasePayload } from "../../services/types";
...
export const CreatePurchaseForm = () => {
 ...
 const { data: inventarioProductos = [] } = useGetInventoryByWarehouseQuery(
  Number(almacenSeleccionado),
  {
   skip: !almacenSeleccionado,
  },
 );
 ...
 useEffect(() => {
  const fetchTipoCambio = async () => {
   if (formState.moneda === MonedaEnum.DOLAR) {
    try {
     const result = await getExchangeRate(formState.fechaEmision).unwrap();
     const tipoCambioValue = result?.compra?.toString() || "3.75";
  ...
  fetchTipoCambio();
 }, [formState.moneda, formState.fechaEmision, getExchangeRate]);
 ...
 const getProductosInventarioOptions = () => {
  const productosEnDetalle = detalleCompra
   .filter(
    (item) =>
     item.almacen ===
     (almacenes.find((a) => a.id.toString() === almacenSeleccionado)?.nombre ||
      almacenSeleccionado),
   )
 ...
 const handleAceptarCompra = async () => {
  ...
  try {
   ...
   await registerPurchase(compraData).unwrap();
 ...
 const handleAceptarYNuevaCompra = async () => {
  ...
  try {
   ...
   await registerPurchase(compraData2).unwrap();
  ...
  <div className={styles.CreatePurchaseForm}>
     ...
     {shouldShowComprobanteAfecto() && (
      ...
      >
       ...
       <ComboBox
        ...
        onChange={(v) =>
         setFormState((prev) => ({ ...prev, idComprobanteAfecto: String(v) }))
        }
       />
      </div>
     )}
      ...
      <>
       ...
       {costosAdicionales && parseFloat(costosAdicionales) > 0 && (
        ...
        >
         <Text size="xs" color="neutral-secondary">
          ...
          {(
           parseFloat(costosAdicionales) /
           detalleCompra.reduce((sum, item) => sum + item.cantidad, 0)
          ).toFixed(2)}{" "}
       ...
       <div className={styles.CreatePurchaseForm__Totals}>
        ...
        >
         ...
         <Text size="sm" color="neutral-primary">
          S/{" "}
          {detalleCompra.reduce((sum, item) => sum + item.total, 0).toFixed(2)}
         </Text>
        </div>
       </div>
      </>
   ...
   <div className={styles.CreatePurchaseForm__Actions}>
    <Button
     ...
     disabled={!areRequiredHeadersComplete() || isLoading || isRegistering}
    >
     Aceptar
    </Button>
    <Button
