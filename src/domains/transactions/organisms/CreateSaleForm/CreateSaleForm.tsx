...
export const CreateSaleForm = () => {
  ...
  if (monedaValue === MonedaEnum.DOLAR) {
   try {
    ...
    setFormState((prev) => ({
     ...prev,
     moneda: monedaValue,
     tipoCambio: result?.compra?.toString() || "3.75", // Usar el valor de compra
    }));
   } catch (error) {
 ...
 const handleAceptarVenta = async () => {
  ...
  try {
   ...
   await registerSale(ventaData).unwrap();
 ...
 const handleAceptarYNuevaVenta = async () => {
  ...
  try {
   ...
   await registerSale(ventaData2).unwrap();
 ...
 // const { data: products = [] } = useGetProductsQuery(); // Unused
 ...
 const handleCreateNewClient = async () => {
  try {
   const response = await createEntity(newClientData).unwrap();
   ...
   if (response.success && response.data) {
    ...
    setNewClientError(response.message || "Error al crear cliente");
   }
  } catch {
      ...
      <>
       ...
       <div className={styles.CreateSaleForm__Totals}>
        ...
        >
         ...
         <Text size="sm" color="neutral-primary">
          S/{" "}
          {detalleVenta.reduce((sum, item) => sum + item.total, 0).toFixed(2)}
         </Text>
        </div>
       </div>
      </>
