...
const MainPage: React.FC = () => {
 const { data: operations = [], isLoading, isError } = useGetOperationsQuery();
 ...
 const [selectedOperation, setSelectedOperation] = useState<Transaction | null>(
  null,
 );
 ...
 const applyAllFilters = () => {
  ...
  if (documentType) {
   filtered = filtered.filter((operation) => {
    ...
    return tipoComprobante.toLowerCase().includes(documentType.toLowerCase());
   });
  }
    ...
    >
     {selectedOperation && (
      <div>
       ...
       <Text size="sm">
        Fecha:{" "}
        {new Date(selectedOperation.fechaEmision).toLocaleDateString("es-PE")}
       </Text>
       <Text size="sm">
        Comprobante:{" "}
       ...
       <Text size="sm">Entidad: {selectedOperation.persona?.razonSocial}</Text>
       <Text size="sm">
        Tipo de Operación: {selectedOperation.tipoOperacion?.descripcion || "-"}
       </Text>
       <Text size="sm">
        Total: S/{" "}
        {parseFloat(selectedOperation.totales?.totalGeneral || "0").toFixed(2)}
       </Text>
      </div>
     )}
    </Modal>
